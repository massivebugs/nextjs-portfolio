...
} from "@/lib/ascii_animation_frames"
...
export default function Top() {
  ...
  const [experienceSectionTextRestore, setExperienceSectionTextRestore] = useState<boolean>(false)
  const [technicalSkillsSectionTextRestore, setTechnicalSkillsSectionTextRestore] =
    useState<boolean>(false)
  const [projectsSectionTextRestore, setProjectsSectionTextRestore] = useState<boolean>(false)
  const [contactMeSectionTextRestore, setContactMeSectionTextRestore] = useState<boolean>(false)
  const [currentPageSection, setCurrentPageSection] = useState<PageSection | null>(null)
  ...
  const switchAnimation = (to: MeAnimation) => {
    if (animation !== MeAnimations.idle && to !== MeAnimations.idle && animation !== null) {
    ...
    <div>
      {animation === MeAnimations.idle ? (
        ...
        <AsciiAnimation
          ...
          frames={ASCII_ANIMATION_FRAMES.concat(ASCII_ANIMATION_FRAMES_DISAPPEAR)}
        ...
        animation === MeAnimations.appear && (
          <AsciiAnimation
            ...
            frames={ASCII_ANIMATION_FRAMES.concat(ASCII_ANIMATION_FRAMES_DISAPPEAR).reverse()}
            start={animationFrameIdx.current}
            frameDurationMs={60}
            flip={flipAnimation}
