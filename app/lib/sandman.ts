...
export class Sandman {
    ...
    return new Promise<Sandman>((resolve) => {
      ...
      const observer = new IntersectionObserver((entries) => {
        entries.forEach((entry, idx) => {
          ...
          const initialTop =
            containerBounds.top + Number(span.dataset[INITIAL_OFFSET_TOP]);
          const initialLeft =
            containerBounds.left + Number(span.dataset[INITIAL_OFFSET_LEFT]);
          const initialBottom =
            containerBounds.top + Number(span.dataset[INITIAL_OFFSET_BOTTOM]);
          const initialRight =
            containerBounds.left + Number(span.dataset[INITIAL_OFFSET_RIGHT]);

          if (
            topX <
            this.container.clientWidth - spanBounds.width - offsetWidth * 2
          ) {
            ...
            leftY <
            this.container.clientHeight - spanBounds.height - offsetHeight * 2
          ) {
      ...
      const spans = Array.from(
        this.container.getElementsByClassName(LETTER_CLASS)
      ).reverse();
      ...
      const observer = new IntersectionObserver((entries) => {
        for (const entry of entries) {
          ...
          const deltaX = this.randomNumber(
            zoneLeft -
              (containerBounds.left +
                Number(span.dataset[INITIAL_OFFSET_LEFT])),
            zoneRight -
              (containerBounds.left +
                Number(span.dataset[INITIAL_OFFSET_RIGHT]))
          );
          ...
          const deltaY = this.randomNumber(
            ...
            zoneBottom -
              (containerBounds.top +
                Number(span.dataset[INITIAL_OFFSET_BOTTOM]))
          );
  ...
  async drop(duration: number = 0.2): Promise<Sandman> {
    ...
    if (
      this.maxDropScrollTop !== 0 &&
      this.container.scrollTop <= this.maxDropScrollTop
    ) {
      return this;
    }
  ...
  private replaceLettersWithSpans() {
    ...
    for (const el of targetElements) {
      ...
      for (const letter of letters) {
        ...
        span.dataset[INITIAL_OFFSET_LEFT] = (
          bounds.left - containerBounds.left
        ).toString();
        span.dataset[INITIAL_OFFSET_RIGHT] = (
  ...
  private calcNewPosition(
    ...
    return {
      y: containerBounds.top + Number(el.dataset[INITIAL_OFFSET_TOP]) + delta.y,
    };
  }
}
